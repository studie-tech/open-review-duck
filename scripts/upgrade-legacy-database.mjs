...
const V0_1_0_BASELINE = {
  createdAt: 1_785_588_462_687,
  hash: "e8fa5c860f7221faf7c4fa6fadc59f7b90e07e6878ce21857098ec7b376e28bf",
};
...
async function legacySchemaDetected(client, baseline) {
  ...
  if (schemaState.rows[0]?.journal_exists !== true) {
    return false;
  }
  ...
  if (result.rows[0]?.baseline_applied === true) return false;
  if (schemaState.rows[0]?.legacy === true) return true;

  const publishedBaseline = await client.query(
    `select exists (
       select 1
       from drizzle.__drizzle_migrations
       where hash = $1 and created_at = $2
     ) as published_legacy`,
    [V0_1_0_BASELINE.hash, V0_1_0_BASELINE.createdAt],
  );
  return publishedBaseline.rows[0]?.published_legacy === true;
}
...
async function addLegacyColumns(client) {
  await client.query(`
    ...
    alter type "ai_completion_reason" add value if not exists 'deep_review_skipped';
    alter type "ai_job_kind" add value if not exists 'review_file';
    alter type "ai_job_kind" add value if not exists 'review_survey';
    alter type "ai_job_kind" add value if not exists 'semantic_cluster';
    ...
    alter table "open_review_duck_ai_job"
      add column if not exists "layoutKey" text,
    ...
    alter table "open_review_duck_provider_connection"
      ...
      add column if not exists "credentialStatus" varchar(24) default 'active' not null,
    ...
    alter table "open_review_duck_review_unit"
      ...
      add column if not exists "changedLineCount" integer default 1 not null;

    alter table "open_review_duck_review_comment"
      add column if not exists "providerCommentExternalId" text,
      add column if not exists "publicationLeaseToken" uuid;

    alter table "open_review_duck_source_blob"
      add column if not exists "uploadLeaseToken" uuid,
      add column if not exists "uploadLeaseExpiresAt" timestamp with time zone,
      add column if not exists "deletionLeaseToken" uuid,
      add column if not exists "deletionLeaseExpiresAt" timestamp with time zone;
  `);
